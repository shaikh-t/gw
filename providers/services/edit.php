...
if (empty($current['id'])) { http_response_code(403); echo 'Forbidden'; exit; }

$id = intval($_GET['id'] ?? 0);
$service = service_find($id);
if (!$service) { http_response_code(404); echo 'Service not found'; exit; }

// ensure current user owns the provider for this service
...
<div class="card mt-4 p-4">
  <h4>Edit service</h4>
  <form method="post" action="/providers/services/update.php" enctype="multipart/form-data">
    <?php echo csrf_field(); ?>
    ...
    <a href="/providers/services/index.php" class="btn btn-link">Back</a>
  </form>
</div>
