...
--
-- Drops in reverse logical order to avoid FK errors
--
...
DROP TABLE IF EXISTS `voice_telemetry_logs`;
...
--
-- Table structure for table `local_knowledge_base`
--
...
--
-- Table structure for table `voice_telemetry_logs`
--

CREATE TABLE `voice_telemetry_logs` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `engine` enum('elevenlabs','native') NOT NULL,
  `characters_used` int(10) unsigned NOT NULL DEFAULT 0,
  `is_error` tinyint(1) NOT NULL DEFAULT 0,
  `error_message` text DEFAULT NULL,
  `server_load` decimal(5,2) NOT NULL DEFAULT 0.00,
  `created_at` timestamp NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `providers` (
  ...
  `deduction_type` enum('percentage','flat') NOT NULL DEFAULT 'percentage',
  `deduction_value` decimal(10,2) NOT NULL DEFAULT 10.00,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uuid` (`uuid`),
  UNIQUE KEY `slug` (`slug`),
...
CREATE TABLE `bot_sessions` (
  ...
  `entry_point` varchar(150) DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NULL DEFAULT NULL ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
...
CREATE TABLE `customer_applications` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_customer_apps_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `customer_documents` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_customer_docs_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `customer_payments` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_customer_payments_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `customer_messages` (
  ...
  `created_at` timestamp NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_customer_messages_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
(18,'footer_disclaimer','Global Ways is a trusted marketplace built for individuals and companies navigating UAE documentation complexity that comes with growth. We work with government-approved vendors who have moved beyond experimentation and now face the challenge of scaling processes, teams, and compliance without losing control. Our approach is execution-led, verified, and grounded in operational reality — not theory. We focus on simplifying ownership, strengthening systems, and aligning leadership around what actually moves your UAE journey forward.','Footer Disclaimer','longtext'),
...
(37,'elevenlabs_api_key','','ElevenLabs API Secret Key','text'),
(38,'elevenlabs_voice_id','21m00Tcm4TlvDq8ikWAM','ElevenLabs Voice ID Mapping','text'),
...
--
-- Dumping data for table `bot_nodes`
--
...
(12, 1, 'voice_selection', 'ar', 'شكراً لك. هل تفضل استكشاف منصتنا بمساعدة الرفيق الصوتي المدعوم بالذكاء الاصطناعي، أم ترغب في تصفح الموقع بشكل مستقل وبسرعتك الخاصة؟', 'شكراً لك. هل تفضل استكشاف منصتنا بمساعدة الرفيق الصوتي المدعوم بالذكاء الاصطناعي، أم ترغب في تصفح الموقع بشكل مستقل وبسرعتك الخاصة؟', NULL),
(13, 1, 'voice_selection', 'ur', 'شکریہ۔ کیا آپ ہمارے AI وائس ساتھی کی مدد سے ہمارے پلیٹ فارم کو دریافت کرنا پسند کریں گے، یا آپ اپنی رفتار سے خود ویب سائٹ دیکھنا پسند کریں گے؟', 'شکریہ۔ کیا آپ ہمارے AI وائس ساتھی کی مدد سے ہمارے پلیٹ فارم کو دریافت کرنا پسند کریں گے، یا آپ اپنی رفتار سے خود ویب سائٹ دیکھنا پسند کریں گے؟', NULL),
(2, 10, 'category_select', 'en', 'Hello! Welcome to Global Ways. We simplify UAE documentation. Please select a service category to get started.', 'Hello! Welcome to Global Ways. We simplify UAE documentation. Please select a service category to get started.', 'fetch_categories'),
(3, 11, 'category_select', 'fr', 'Bonjour! Bienvenue sur Global Ways. Nous simplifions les démarches administratives aux Émirats Arabes Unis. Veuillez sélectionner une catégorie de service pour commencer.', 'Bonjour! Bienvenue sur Global Ways. Nous simplifions les démarches administratives aux Émirats Arabes Unis. Veuillez sélectionner une catégorie de service pour commencer.', 'fetch_categories'),
(4, 12, 'category_select', 'ar', 'مرحباً بك في غلوبال وايز! نحن نبسط الإجراءات والمعاملات الرسمية في دولة الإمارات العربية المتحدة. يرجى اختيار قسم الخدمة للبدء.', 'مرحباً بك في غلوبال وايز! نحن نبسط الإجراءات والمعاملات الرسمية في دولة الإمارات العربية المتحدة. يرجى اختيار قسم الخدمة للبدء.', 'fetch_categories'),
(5, 13, 'category_select', 'ur', 'گلوبل ویز میں خوش آمدید! ہم متحدہ عرب امارات کے کاغذی کام کو آسان بناتے ہیں۔ شروع کرنے کے لیے برائے مہربانی ایک سروس کیٹیگری منتخب کریں۔', 'گلوبل ویز میں خوش آمدید! ہم متحدہ عرب امارات کے کاغذی کام کو آسان بناتے ہیں۔ شروع کرنے کے لیے برائے مہربانی ایک سروس کیٹیگری منتخب کریں۔', 'fetch_categories'),
(20, 10, 'independent_browse', 'en', 'Understood. I will remain silently available in the bottom corner of your screen. As you browse our services, I will automatically keep track of your progress so that whenever you require guidance, we can seamlessly pick up exactly where you left off.', 'Understood. I will remain silently available in the bottom corner of your screen. As you browse our services, I will automatically keep track of your progress so that whenever you require guidance, we can seamlessly pick up exactly where you left off.', 'independent_browse'),
(21, 11, 'independent_browse', 'fr', 'Compris. Je resterai discrètement disponible dans le coin inférieur de votre écran. Pendant votre navigation, je suivrai automatiquement votre progression afin que, dès que vous aurez besoin de conseils, nous puissions reprendre exactement là où vous vous étiez arrêté.', 'Compris. Je resterai discrètement disponible dans le coin inférieur de votre écran. Pendant votre navigation, je suivrai automatiquement votre progression afin que, dès que vous aurez besoin de conseils, nous puissions reprendre exactement là où vous vous étiez arrêté.', 'independent_browse'),
